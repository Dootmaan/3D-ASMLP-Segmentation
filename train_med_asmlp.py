import torch as pt
import numpy as np
from model.asmlp import Med_ASMLP
...
trainset=BraTSDataset3D('/path/to/BraTS20',mode='train')
# valset=BraTSDataset3D('/path/to/BraTS20',mode='val')
testset=BraTSDataset3D('/path/to/BraTS20',mode='test')
...
model= Med_ASMLP().cuda()
# model.load_state_dict(pt.load(model_path+'/Med_ASMLP_3D_ras_noalter_BraTS_patch-free_bs1_best.pt',map_location = 'cpu'))
...
#     model=TransLite3D(in_channels=1,out_channels=1).cuda()
...
for x in range(epoch):
    ...
    for i,data in enumerate(train_dataset):
        ...
        if i%10==0:
            ...
            cv2.imwrite('MedMLP_3d_patchfree_combine.png',final_img)
    ...
    if dice>best_dice:
        best_dice=dice
        print('New best dice! Model saved to',model_path+'/Med_ASMLP_3D_ras_noalter_BraTS_patch-free_bs'+str(batch_size)+'_best.pt')
        pt.save(model.state_dict(), model_path+'/Med_ASMLP_3D_ras_noalter_BraTS_patch-free_bs'+str(batch_size)+'_best.pt')
    # print('===TEST===>')
    # TestModel() 
